...
class InputSwitcher:
    ...
    async def switch_input(
        ...
        try:
            ...
            logger.info(
                f"Switching from slot {self._active_slot} to slot {next_slot}: " f"{new_video_path}"
            )
            ...
            logger.info(f"Input switched successfully (switch #{self._switch_count})")
            return True
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "active_path": (
                self._slots[self._active_slot].path if self._slots[self._active_slot] else None
            ),
        }
