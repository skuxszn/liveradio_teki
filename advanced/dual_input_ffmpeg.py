...
class DualInputFFmpegManager:
    ...
    async def start_stream(
        ...
        try:
            ...
            if self._process.poll() is None:
                ...
                logger.info(
                    f"FFmpeg stream started successfully (PID: {self._process.pid})"
                )
                return True
            else:
    ...
    async def switch_track(
        ...
        try:
            ...
            self._state = StreamState.RUNNING
            logger.info(
                f"Track switch completed (#{self._switch_count}): {new_loop}"
            )
            return True
    ...
    def _build_command(
        ...
        cmd = [
            ...
            "-bufsize", f"{int(self.config.video_bitrate.rstrip('k')) * 2}k",
        ...
        while self._should_monitor:
            try:
                ...
                if self._process.poll() is not None:
                    logger.error(
                        f"FFmpeg process crashed! Exit code: {self._process.returncode}"
                    )
                    self._state = StreamState.ERROR
                ...
                try:
                    ...
                    logger.debug(
                        f"FFmpeg (PID {self._process.pid}): "
