...
class HLSManager:
    ...
    async def _start_encoder(
        ...
        cmd = [
            ...
            "-hls_segment_filename", str(self.segments_dir / "segment_%03d.ts"),
    ...
    def get_status(self) -> dict:
        ...
        return {
            "encoder_running": (
                self._encoder_process is not None
                and self._encoder_process.poll() is None
            ),
            "streamer_running": (
                self._streamer_process is not None
                and self._streamer_process.poll() is None
            ),
            "encoder_pid": (
                self._encoder_process.pid if self._encoder_process else None
            ),
            "streamer_pid": (
                self._streamer_process.pid if self._streamer_process else None
            ),
