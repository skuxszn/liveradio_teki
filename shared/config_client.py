...
class DashboardConfigClient:
            ...
            try:
                async with httpx.AsyncClient(timeout=10.0) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.error(
                            f"[{self.service_name}] Failed to fetch config: HTTP {response.status_code}"
                        )
                        return None
    ...
    async def start_auto_refresh(self, on_change: Optional[Callable[[Dict, Dict], None]] = None):
        ...
        """
        logger.info(
            f"[{self.service_name}] Starting auto-refresh (interval: {self.refresh_interval}s)"
        )
