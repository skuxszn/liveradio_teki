...
def export_play_history(
    ...
    with engine.connect() as conn:
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                'ID', 'Track Key', 'Artist', 'Title', 'Album',
                'AzuraCast Song ID', 'Loop File Path',
                'Started At', 'Ended At', 'Duration (seconds)',
                'Expected Duration (seconds)', 'FFmpeg PID',
                'Had Errors', 'Error Message', 'Error Count'
            ])
...
def export_system_metrics(
    ...
    with engine.connect() as conn:
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                'ID', 'Timestamp', 'Metric Name', 'Metric Value',
                'Unit', 'Service'
            ])
...
Examples:
  ...
  python scripts/export_analytics.py --output-dir /tmp/reports
    ...
    parser.add_argument(
        '--include-errors',
        action='store_true',
        help='Include error log export'
    )
    parser.add_argument(
        '--include-metrics',
        action='store_true',
        help='Include system metrics export'
    )
    ...
    try:
        ...
        summary_file = os.path.join(
            args.output_dir,
            f'analytics_summary_{timestamp}.csv'
        )
