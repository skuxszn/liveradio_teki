...
async def get_mapping_stats(current_user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return {"total_mappings": total, "most_played": most_played, "recently_added": recently_added}


@router.get("/{mapping_id}", response_model=dict)
async def get_mapping(
    mapping_id: int, current_user=Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    if not result:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Mapping not found: {mapping_id}"
        )
...
async def update_mapping(
    ...
    # Check if exists
    check = db.execute(
        text("SELECT id FROM track_mappings WHERE id = :id"), {"id": mapping_id}
    ).fetchone()
...
async def delete_mapping(
    ...
    # Delete mapping
    db.execute(text("DELETE FROM track_mappings WHERE id = :id"), {"id": mapping_id})
    db.commit()
...
async def bulk_delete_mappings(
    ...
    for mapping_id in mapping_ids:
        result = db.execute(text("DELETE FROM track_mappings WHERE id = :id"), {"id": mapping_id})
...
async def export_mappings(current_user=Depends(get_current_user), db: Session = Depends(get_db)):
