...
async def websocket_endpoint(websocket: WebSocket, client_id: Optional[str] = Query(None)):
    ...
    await manager.send_personal_message(
        {
            "type": "connected",
            "data": {"client_id": client_id, "message": "Connected to dashboard WebSocket"},
        },
        websocket,
    )
            ...
            try:
                ...
                if message.get("type") == "subscribe":
                    ...
                    if event_type:
                        ...
                        await manager.send_personal_message(
                            {"type": "subscribed", "data": {"event_type": event_type}}, websocket
                        )
                    ...
                    if event_type:
                        ...
                        await manager.send_personal_message(
                            {"type": "unsubscribed", "data": {"event_type": event_type}}, websocket
                        )

                elif message.get("type") == "ping":
                    # Respond to ping with pong
                    await manager.send_personal_message({"type": "pong", "data": {}}, websocket)
