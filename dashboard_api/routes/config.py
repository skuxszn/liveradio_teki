...
async def export_settings_internal(request: Request, db: Session = Depends(get_db)):
...
async def get_settings_by_category(
    category: str, current_user=Depends(get_current_user), db: Session = Depends(get_db)
):
...
async def update_setting(
    ...
    if not setting:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Setting not found: {category}.{key}"
        )
    ...
    auth_service.log_audit(
        ...
        details={
            "old_value": old_value,
            "new_value": update_data.value if not setting.is_secret else "***",
        },
...
async def export_configuration(current_user=Depends(require_admin), db: Session = Depends(get_db)):
...
async def test_azuracast_connection(
    request: Request, current_user=Depends(require_operator), db: Session = Depends(get_db)
):
    ...
    azuracast_url = (
        db.query(Setting)
        .filter(Setting.category == "stream", Setting.key == "AZURACAST_URL")
        .first()
    )

    azuracast_api_key = (
        db.query(Setting)
        .filter(Setting.category == "stream", Setting.key == "AZURACAST_API_KEY")
        .first()
    )

    if not azuracast_url or not azuracast_url.value:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="AzuraCast URL not configured"
        )

    if not azuracast_api_key or not azuracast_api_key.value:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="AzuraCast API key not configured"
        )
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0, follow_redirects=True) as client:
            response = await client.get(
                f"{azuracast_url.value}/api/status", headers={"X-API-Key": azuracast_api_key.value}
            )
...
async def generate_security_token(
    ...
    # Update setting
    setting = (
        db.query(Setting).filter(Setting.category == "security", Setting.key == setting_key).first()
    )
